...
public class Logic {
    ...
    Trainer trainer;
     ...
     List<Course> coursessyn= new ArrayList<>();
     List<Trainer> trainerssyn;//= new ArrayList<>();
     List<Assignment> assignssyn;//= new ArrayList<>();
     List<Student> stsyn;//= new ArrayList<>();
...
//-----Show manually entries Menu------------------
    ...
    public void showManuallyEntriesMenu() {
        System.out.println("Show manual entries ");
...
//  DateTimeFormatter formatter = DateTimeFormatter.ofPattern("d/MM/yyyy");
  ...
  //      String date2 = "17/08/2017"; //use this
            ...
            for (Student st : crs.getStudent_Assignment_Course().keySet()) {
                for (List<Assignment> listt : crs.getStudent_Assignment_Course().values()) {
                    ...
                    while (itr.hasNext()) {
                        ...
                        if(itr.hasNext()){
                           String st1= itr.next().getTitle();
                        LocalDate localDate1 = LocalDate.parse(st2, formatter);
                        if ((dateFunction(localDate1, localDate2)) == true) {
                            assignn=true;
                            System.out.println(st.getfName()+" " +st.getlName()+ " Assignment date on: "+ localDate1 +" Assignment title: "+ st1);
                        }}
...
//-------------------------------Enter manually entries (Press 1)";
    public void addEntities() {
        ...
        student_Assignment_Course.put(student, assignssyn);
        course.setStudent_Assignment_Course(student_Assignment_Course);
    ...
    public void addStudent() {
        ...
        student = new Student(name, lname, dateOfBirth);
        stsyn =new ArrayList<>();
        stsyn.add(student);
    ...
    public void addTrainer() {
        ...
        trainerssyn.add(trainer);
        course.setTrainers(trainerssyn);
...
//    Asignment lists to course
        ...
        assignsyn2.add(md.assign1);
        assignsyn2.add(md.assign2);
         assignsyn2.add(md.assign4);
        assignsyn3.add(md.assign4);
        //assignssyn2.add(md.assign1);
...
//assignment list2md.
        ...
        student_Assignment_Course2.put(md.st2, assignsyn2);
         student_Assignment_Course2.put(md.st4, assignsyn3);
         student_Assignment_Course2.put(md.st1, assignssyn);
        ...
        student_Assignment_Course3.put(md.st2, assignsyn3);
...
//show all trainers
        ...
        for (Course crs : coursessyn) {
            ...
            for (Iterator<Trainer> it = crs.getTrainers().iterator(); it.hasNext();) {
                Trainer tr = it.next();
                System.out.println(tr);
            }
        }

    }   else{ System.out.println("PLEASE ENTER VALUES FIRST");}
...
Set<Trainer> hashSet = new HashSet<Trainer>();
for (Course crs : coursessyn) {
...
for (int i = 0; i < abc.size(); i++) {
    if (set.contains(abc.get(i))) {
        System.out.println(abc.get(i) + " duplicates");
    } else {
        set.add(abc.get(i));
    }
}
...
for (Course crs : coursessyn) {
    ...
    for (Student st : crs.getStudent_Assignment_Course().keySet()) {
        System.out.println(st.toString() + " " + crs.getStudent_Assignment_Course().get(st));
            ...
            System.out.println("PLEASE ENTER VALUES FIRST");
        }}
